...
fn main() {
    ...
    let a = Matrix::from_vec(10000, 1000, (0..10000*1000).collect());
    let b = Matrix::from_vec(10000, 1000, (0..10000*1000).map(|x| x*2).collect());

    let a_cl = ClMatrix::from_matrix(ctx, &a, ClMatrixMode::In);
    let b_cl = ClMatrix::from_matrix(ctx, &b, ClMatrixMode::In);
    let c_cl: ClMatrix<usize> = ClMatrix::new(ctx, 10000, 1000, ClMatrixMode::Out);
